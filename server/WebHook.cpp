...
const string kOnSvacRecord = HOOK_FIELD "on_svac_record";
...
void installWebHook() {
    ...
    // SVAC RTP dump 文件关闭后通知后端
    NoticeCenter::Instance().addListener(&web_hook_tag, Broadcast::kBroadcastRecordSvacRtp, [](BroadcastRecordSvacRtpArgs) {
        GET_CONFIG(string, hook_svac_record, Hook::kOnSvacRecord);
        if (!hook_enable || hook_svac_record.empty()) {
            return;
        }
        ArgsType body;
        ...
        do_http_hook(hook_svac_record, body, nullptr);
    });

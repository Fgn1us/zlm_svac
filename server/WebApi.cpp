 ...
 */
#if defined(ENABLE_RTPPROXY)
// 跨平台获取磁盘空间（path 所在文件系统的总空间和剩余空间）
static bool getDiskSpace(const std::string &path, uint64_t &total_bytes, uint64_t &free_bytes) {
#ifdef _WIN32
    ULARGE_INTEGER freeAvailable, total, totalFree;
    if (!GetDiskFreeSpaceExA(path.c_str(), &freeAvailable, &total, &totalFree)) {
        return false;
    }
    total_bytes = total.QuadPart;
    free_bytes = totalFree.QuadPart;
#else
    struct statvfs st;
...
void installWebApi() {
    ...
    api_regist("/index/api/getStorageList", [](API_ARGS_MAP) {
        CHECK_SECRET();
    ...
    api_regist("/index/api/setStoragePath", [](API_ARGS_MAP) {
        CHECK_SECRET();
        CHECK_ARGS("path");

        std::string path = allArgs["path"];

...
// PlaybackSVAC instance management
static std::mutex s_playback_mtx;
static std::map<std::string, mediakit::PlaybackSVAC::Ptr> s_playback_map;

static std::string makePlaybackKey(const std::string &vhost, const std::string &app, const std::string &stream) {
...
void installWebApi() {
    ...
    api_regist("/index/api/startPlaybackSVAC", [](API_ARGS_MAP_ASYNC) {
        CHECK_SECRET();
        CHECK_ARGS("vhost", "app", "stream", "file_path", "ssrc", "dst_url", "dst_port");

        auto key = makePlaybackKey(allArgs["vhost"], allArgs["app"], allArgs["stream"]);

        MediaSourceEvent::SendRtpArgs args;
        args.ssrc = allArgs["ssrc"];
        args.dst_url = allArgs["dst_url"];
        args.dst_port = allArgs["dst_port"].as<int>();
        args.pt = allArgs["pt"].empty() ? 96 : allArgs["pt"].as<int>();
        args.src_port = allArgs["src_port"];
        args.con_type = allArgs["is_udp"].empty() || allArgs["is_udp"].as<bool>() ?
            MediaSourceEvent::SendRtpArgs::kUdpActive :
            MediaSourceEvent::SendRtpArgs::kTcpActive;
        auto file_path = allArgs["file_path"];

        auto poller = EventPollerPool::Instance().getPoller();
        auto playback = std::make_shared<mediakit::PlaybackSVAC>(poller);

        std::weak_ptr<mediakit::PlaybackSVAC> weak_playback = playback;
        playback->setOnClose([key](const SockException &) {
            std::lock_guard<std::mutex> lck(s_playback_mtx);
            s_playback_map.erase(key);
        });

        playback->start(file_path, args, [val, headerOut, invoker, key, weak_playback](uint16_t local_port, const SockException &ex) mutable {
            if (ex) {
                val["code"] = API::OtherFailed;
                val["msg"] = ex.what();
                invoker(200, headerOut, val.toStyledString());
            ...
            {
                std::lock_guard<std::mutex> lck(s_playback_mtx);
                if (auto strong_playback = weak_playback.lock()) {
                    s_playback_map[key] = strong_playback;
                }
            }
            val["code"] = API::Success;
            val["msg"] = "playbackSVAC started successfully";
            val["local_port"] = local_port;
            invoker(200, headerOut, val.toStyledString());
        });
    });

    api_regist("/index/api/stopPlaybackSVAC", [](API_ARGS_MAP_ASYNC) {
        CHECK_SECRET();
        CHECK_ARGS("vhost", "app", "stream");

        auto key = makePlaybackKey(allArgs["vhost"], allArgs["app"], allArgs["stream"]);
        mediakit::PlaybackSVAC::Ptr playback;
        {
            std::lock_guard<std::mutex> lck(s_playback_mtx);
            auto it = s_playback_map.find(key);
            if (it == s_playback_map.end()) {
                throw ApiRetException("can not find playback svac for this stream", API::NotFound);
            }
            playback = it->second;
            s_playback_map.erase(it);
        }
        playback->stop();
        val["code"] = API::Success;
        val["msg"] = "stopPlaybackSVAC called successfully";
        invoker(200, headerOut, val.toStyledString());
    });

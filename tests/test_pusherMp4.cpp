...
public:
    ...
    bool start() {
        try {
            ...
            NoticeCenter::Instance().addListener(tag.get(), Broadcast::kBroadcastMediaChanged, [this, poller, tag](BroadcastMediaChangedArgs) {
                // 检查是否是我们关心的流注册了
                if (bRegist && sender.getMediaTuple().vhost == tuple_.vhost
                    && sender.getMediaTuple().app == tuple_.app
                    && sender.getMediaTuple().stream == tuple_.stream) {
                    InfoL << "媒体源已就绪，开始推流";
                    ...
                    // 移除监听器
                    NoticeCenter::Instance().delListener(tag.get(), Broadcast::kBroadcastMediaChanged);
                }
            });
...
int main(int argc, char *argv[]) {
    ...
    auto status_timer = std::make_shared<Timer>(10.0f, []() {
        ...
        if (g_relays.empty()) {
            ...
            cout << "====== 代理状态 (" << getTimeStr("%Y-%m-%d %H:%M:%S") << ") ======" << endl;

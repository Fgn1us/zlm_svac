...
// SVAC RTP dump 文件关闭后广播（用于通知后端录像完成）
// Broadcast after SVAC RTP dump file is closed (for notifying backend of recording completion)
struct SvacRecordInfo {
    std::string stream;
    std::string file_name;
    std::string file_path;
    uint64_t file_size;
    int start_time; // 当天分钟数 (minutes since midnight)
};
extern const std::string kBroadcastRecordSvacRtp;
#define BroadcastRecordSvacRtpArgs const Broadcast::SvacRecordInfo &info
...
// 用于验签的证书文件路径  [AUTO-TRANSLATED:1f3f4e2d]
extern const std::string kVerifyCertFilePath;
} // namespace RtpProxy

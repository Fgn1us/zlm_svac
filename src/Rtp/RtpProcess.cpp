...
RtpProcess::~RtpProcess() {
    ...
    // 通知后端录制文件完成
    if (_auto_enabled) {
        emitRecordNotify(_save_file_rtp_auto, _auto_dump_file_path, _auto_dump_file_name, _auto_dump_start_time);
    }
...
void RtpProcess::openRtpDumpFile(const std::string &prefix, std::shared_ptr<FILE> &file) {
    ...
    if (file) {
        ...
        // 追踪自动录制文件信息
        if (prefix.empty()) {
            _auto_dump_file_path = path;
            // 提取文件名（路径最后一段）
            auto pos = _auto_dump_file_path.rfind('/');
            _auto_dump_file_name = (pos != string::npos) ? _auto_dump_file_path.substr(pos + 1) : _auto_dump_file_path;
            _auto_dump_start_time = std::chrono::system_clock::to_time_t(now);
        }
    } else {
...
void RtpProcess::emitRecordNotify(std::shared_ptr<FILE> &file, const std::string &file_path,
                                   const std::string &file_name, std::time_t start_time) {
    if (!file || file_path.empty()) return;
    ...
    // 计算 start_time: 当天分钟数
    std::tm *lt = std::localtime(&start_time);
    int minutes_since_midnight = lt->tm_hour * 60 + lt->tm_min;

    Broadcast::SvacRecordInfo info;
    info.stream = _media_info.stream;
    info.file_name = file_name;
    info.file_path = file_path;
    info.file_size = (uint64_t)file_size;
    info.start_time = minutes_since_midnight;

    InfoL << "[RtpProcess] emit record notify: stream=" << info.stream
          << ", file=" << info.file_name << ", size=" << info.file_size
          << ", start_time=" << info.start_time << "min";

    try {
        NOTICE_EMIT(BroadcastRecordSvacRtpArgs, Broadcast::kBroadcastRecordSvacRtp, info);
    } catch (std::exception &ex) {
        WarnL << "[RtpProcess] emit record notify error: " << ex.what();
    }
}
